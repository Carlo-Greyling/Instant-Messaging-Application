...
/* harmony import */ var angular_agora_rtc__WEBPACK_IMPORTED_MODULE_18__ = __webpack_require__(/*! angular-agora-rtc */ "./node_modules/angular-agora-rtc/fesm2015/angular-agora-rtc.js");
...
let ChatWindowComponent = class ChatWindowComponent {
    constructor(dialog, snackBar, firebaseService) {
        this.dialog = dialog;
    ...
    OpenMultiMedia() {
        ...
        const dialogRef = this.dialog.open(_multimedia_multimedia_component__WEBPACK_IMPORTED_MODULE_4__["MultimediaComponent"], {
    ...
    openSettings() {
        ...
        const right = bodyRect.right - elemRect.right;
        const top = elemRect.top - bodyRect.top;
        const dialogRef = this.dialog.open(_view_settings_viewsettings_component__WEBPACK_IMPORTED_MODULE_6__["ViewsettingsComponent"], {
            // This is only if we wish to send parameters to the popup component
            data: {
                UserId: 0,
                ContactId: 0
    ...
    onGenerateNewMessage() {
        ...
        const newMessage = new _shared_messages_model__WEBPACK_IMPORTED_MODULE_2__["Messages"](this.thisUserID, this.newMessage, 'msgText', this.msgHours + ':' + this.msgMinutes);
...
let MultimediaComponent = class MultimediaComponent {
    constructor(snackBar, dialogRef, data, encodeService, firebaseService) {
        this.snackBar = snackBar;
...
class EncoderService {
    constructor(firebaseService, lz) {
        this.firebaseService = firebaseService;
        this.lz = lz;
    ...
    Base64EncodeImage(theFile, fileName, UserId, ContactId, active) {
    ...
    Base64EncodeVideo(theFile, fileName, UserId, ContactId, active) {
        this.activeContact = active;
        const reader = new FileReader();
        reader.readAsDataURL(theFile);
        let result;
    ...
    Base64EncodeAudio(theFile, fileName, UserId, ContactId, active) {
        this.activeContact = active;
        const reader = new FileReader();
        reader.readAsDataURL(theFile);
        let result;
    ...
    DecryptTextMessage(encryptedMessage) {
    ...
    onGenerateNewImageMessage(imageBase64String) {
        ...
        const newMessage = new _messages_model__WEBPACK_IMPORTED_MODULE_4__["Messages"](localStorage.getItem('currentUserId'), base64string, 'msgImage', this.msgHours + ':' + this.msgMinutes);
        this.message.unshift(newMessage);
        this.firebaseService.newMessage(newMessage, this.activeContact);
    }
    onGenerateNewVideoMessage(imageBase64String) {
        const base64string = this.lz.compress(imageBase64String);
        this.today = new Date();
        this.msgMinutes = this.today.getMinutes();
        this.msgHours = this.today.getHours();
        ...
        const newMessage = new _messages_model__WEBPACK_IMPORTED_MODULE_4__["Messages"](localStorage.getItem('currentUserId'), base64string, 'msgVideo', this.msgHours + ':' + this.msgMinutes);
        this.message.unshift(newMessage);
        this.firebaseService.newMessage(newMessage, this.activeContact);
    }
    onGenerateNewAudioMessage(imageBase64String) {
        const base64string = this.lz.compress(imageBase64String);
        this.today = new Date();
        this.msgMinutes = this.today.getMinutes();
        this.msgHours = this.today.getHours();
        ...
        const newMessage = new _messages_model__WEBPACK_IMPORTED_MODULE_4__["Messages"](localStorage.getItem('currentUserId'), base64string, 'msgAudio', this.msgHours + ':' + this.msgMinutes);
        this.message.unshift(newMessage);
        this.firebaseService.newMessage(newMessage, this.activeContact);
    }
    profilePicBase64(theFile, fileName) {
        // Do Conversion
...
let FirebaseService = class FirebaseService {
    ...
    createUser(userId, name, password) {
        ...
        const ChatsIdArr = []; // 0123456789_9876543210
        const openChatUserIds = []; // 0123456789
        const data = {
            name,
            status: 'online',
                ...
                for (let i = 0; i < this.openChatUserIds.length; i++) {
                    ...
                    const document = ref.get().toPromise()
                        ...
                        if (!udoc.exists) {
                            ...
                            this.userProfilesArr.push(new _users_model__WEBPACK_IMPORTED_MODULE_5__["Users"](udoc.data().userId, udoc.data().name, udoc.data().status, udoc.data().profilePicture, udoc.data().onlineIcon, doc.data().openChatUserIds));
                        }
                    }).catch(err => {
                        console.log('Error', err); // add toastr notification
                    });
        ...
        const getDoc = chatsRef.get().toPromise()
            ...
            if (!doc.exists) {
                ...
                for (let i = 0; i < msgIdArray.length; i++) {
                    this.messagesArr.push(new _messages_model__WEBPACK_IMPORTED_MODULE_4__["Messages"](msgIdArray[i], msgContentsArray[i], msgTypeArray[i], arrTimeArray[i]));
                }
            }
        }).catch(err => {
            console.log('Error', err); // add toastr notification
    ...
    getMessagesWithOtherChatId(friendId) {
        let arrTimeArray = [];
        let msgContentsArray = [];
        let msgIdArray = [];
        let msgTypeArray = [];
    ...
    newMessage(newMessage, friendId) {
        let arrTimeArray = [];
        let msgContentsArray = [];
        let msgIdArray = [];
        let msgTypeArray = [];
    ...
    newMessageWithOtherChatId(newMessage, friendId) {
        let arrTimeArray = [];
        let msgContentsArray = [];
        let msgIdArray = [];
        let msgTypeArray = [];
    ...
    newChat(userId) {
        ...
        let myUserID = '';
        let password = '';
        let ChatsIdArr = []; // 0123456789_9876543210
        let openChatUserIds = []; // 0123456789
        const userRef = this.db.collection('users').doc(localStorage.getItem('currentUserId'));
        const getDoc = userRef.get().toPromise()
            .then(doc => {
            if (!doc.exists) {
...
class Messages {
    constructor(msgID, msgContents, msgType, arrivalTime) {
        this.msgID = msgID;
...
class Users {
    constructor(userID, name, status, profilePicture, onlineIcon, openChatUserIds) {
        this.userID = userID;
        this.name = name;
        this.status = status;
...
let SignupComponent = class SignupComponent {
    constructor(snackBar, firebaseService, encodeService) {
        this.snackBar = snackBar;
        this.firebaseService = firebaseService;
        this.encodeService = encodeService;
        this.base64DP = '';
...
/* harmony import */ var tslib__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! tslib */ "./node_modules/tslib/tslib.es6.js");
/* harmony import */ var _angular_core__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! @angular/core */ "./node_modules/@angular/core/fesm2015/core.js");
/* harmony import */ var angular_agora_rtc__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! angular-agora-rtc */ "./node_modules/angular-agora-rtc/fesm2015/angular-agora-rtc.js");



let VideoCallingComponent = class VideoCallingComponent {
...
VideoCallingComponent.ctorParameters = () => [
...
let ViewsettingsComponent = ViewsettingsComponent_1 = class ViewsettingsComponent {
    constructor(snackBar, dialogRef, data, dialog) {
        this.snackBar = snackBar;
        this.dialogRef = dialogRef;
        this.data = data;
        this.dialog = dialog;
    ...
    selectTheme() {
        ...
        const right = bodyRect.right - elemRect.right;
        const top = elemRect.top - bodyRect.top;
        const dialogRef = this.dialog.open(ViewsettingsComponent_1, {
            // This is only if we wish to send parameters to the popup component
            data: {
                UserId: 0,
                ContactId: 0
...
let VoiceCallingComponent = class VoiceCallingComponent {
    constructor(agoraService) {
        this.agoraService = agoraService;
        this.remoteCalls = [];
        this.agoraService.createClient();
...
VoiceCallingComponent.ctorParameters = () => [
    { type: angular_agora_rtc__WEBPACK_IMPORTED_MODULE_2__["AngularAgoraRtcService"] }
];
